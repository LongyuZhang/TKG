		...
		if($file) {
			while ($file =~ /System dump written to (.*)/g) {
				...
				if(!exists $parsedNames{$curCoreName}) {
					...
					my $curCoreAbsPath = Cwd::abs_path( $curCorePath );
					my $moveLocationAbs = Cwd::abs_path( $moveLocation );

					#test only
					logMsg("curCoreAbsPath is  $curCoreAbsPath");
					logMsg("moveLocationAbs is   $moveLocationAbs");
					
					if($curCoreAbsPath !~  $moveLocationAbs) {
						qx(mv '$curCoreAbsPath' '${moveLocation}');
						my $moveResult = $?;
			...
			# test only
			my $listCores = "ls -al /cores";
			my @coreFiles = qx($listCores);
			print("original /cores folder: @coreFiles \n");

			my $listRoot = "ls -al ".${testRoot}."";
			my @rootFiles = qx($listRoot);
			print("destination ${testRoot} folder: @rootFiles \n");

			my $listTargetsUp = "ls -al "${moveLocation}/.."";
			my @destFiles2 = qx($listTargetsUp);
			print("destination ${moveLocation}/.. folder: @destFiles2 \n");
