...
ifneq (, $(findstring current, $(JCL_VERSION)))
	ifeq (8, $(JDK_VERSION))
		...
		ifeq (default, $(VM_SUBDIR))
			...
			CHECK_JAVA_SHARED_LIBRARIES_DIR=JAVA_SHARED_LIBRARIES_DIR=$(JAVA_SHARED_LIBRARIES_DIR); echo $(Q)$(JVM_OPTIONS)$(Q) | grep -q -- '-Xcompressedrefs' && JAVA_SHARED_LIBRARIES_DIR=$$(echo $(Q)$$JAVA_SHARED_LIBRARIES_DIR$(Q) | sed 's|/default|/compressedrefs|g; s|\\default|\\compressedrefs|g')
		endif
		ADD_JVM_LIB_DIR_TO_LIBPATH=$(CHECK_VM_SUBDIR) ; echo Set LIBPATH to $$LIBPATH_VAL && export LIBPATH=$$LIBPATH_VAL; $(CHECK_JAVA_SHARED_LIBRARIES_DIR); echo Set JAVA_SHARED_LIBRARIES_DIR to $$JAVA_SHARED_LIBRARIES_DIR && export JAVA_SHARED_LIBRARIES_DIR=$$JAVA_SHARED_LIBRARIES_DIR && cd $$JAVA_SHARED_LIBRARIES_DIR;
	else
		ifneq (,$(findstring win,$(SPEC)))
